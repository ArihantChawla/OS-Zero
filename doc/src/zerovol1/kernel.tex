...
\begin{tabular}{ | l | l | l | }
	...
	Macro        & Brief & Notes \\
	\hline
	\textbf{SMP} & symmetric multiprocessor support & Not functional yet \\
	\hline
	\textbf{HZ}  & scheduler timer frequency        & default value is 250 \\
	\hline
	\textbf{ZEROSCHED} & default thread scheduler   & do not change yet \\
	\hline
	\textbf{NPROC}     & maximum number of simultaneous processes & default is 256 \\
	\hline
	\textbf{NTHR}      & maximum number of simultaneous threads   & default is 4096 \\
	\hline
...
\subsection{Segment Descriptor Tables}
	...
	address in GDT or a local one with the address in LDT. Processes are
	required to declare a handful of entries into their descriptor tables.
...
\subsection{System Call Mechanism}
	...
	On IA-32 architectures, all of up to 3 system call parameters are passed
	in registers; the system call number is in \textbf{EAX}. On
	\textbf{X86-64}	all system call arguments are passed in registers. On
	both architectures, system calls return a value for \textbf{errno};
	\textbf{failures} are indicated by setting the \textbf{CF-bit} (carry)
	in the \textbf{EFLAGS}-register. System calls are, in the first
...
\begin{tabular}{ | l | l | l | l | }
	...
	\hline
	\textbf{THR\_SETATTR} & thrid & struc thrattr *  & set other attributes \\
	\hline
\end{tabular}
...
\subsection{pctl}
	...
	The pctl system call provides process operations. The following table
	lists possible values for the \textbf{cmd} argument.

\begin{tabular}{ | l | l | l | l | }
	\hline
	\textbf{cmd}    & \textbf{parm} & \textbf{arg} & Notes \\
	\hline
	\textbf{PROC\_GETPID}    & N/A           & N/A & getpid() \\
	\hline
	\textbf{PROC\_GETPGRP}   & N/A           & N/A & getpgrp() \\
	\hline
	\textbf{PROC\_WAIT}      & procid        & N/A & wait() \\
	                & \textbf{PROC\_WAITPID} & N/A & wait for pid \\
                        & \textbf{PROC\_WAITCLD} & N/A & wait for children in the group pid \\
	                & \textbf{PROC\_WAITGRP} & N/A & wait for children in the group of caller \\
	                & \textbf{PROC\_WAITANY} & N/A & wait for any child process \\
	\hline
	\textbf{PROC\_USLEEP}    & milliseconds  & N/A & usleep() \\
	\hline
	\textbf{PROC\_NANOSLEEP} & nanoseconds   & N/A & nanosleep() \\
	\hline
\end{tabular}
...
\begin{tabular}{ | l | l | }
	...
	\hline
	\textbf{MAP\_SINGLE}     & map buffer mapped to single user process and kernel \\
	\hline
