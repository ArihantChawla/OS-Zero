...
\chapter{Overview}
	...
	V0 words are little-endian (LSB) in byte-order; the rationale includes
        easy ``casting'' of scalar types to smaller ones by just accessing the
        low-order bytes without need for address calculations.
...
\begin{tabular}{ | l | l | l | l | }
        ...
        \hline
        MSW  & Machine Status Word & Processor Features & Low 16 Bits (POP) \\
        \hline
        MFW  & Machine Feature Word & Processor Features & Low 16 Bits (POP) \\
        \hline
\end{tabular}
...
\section{Input-Output}
        ...
        \textbf{Brief}
        V0 implements I/O with a separate address space of ``ports''. The range
        is 0-1023; this should be adequate for a minimalistic system and the
        range (opcode val-field) may be bigger in other Valhalla processors.
...
\begin{tabular}{ | l | l | l | l | }
        ...
        \hline
        CMP      & 0x03      & Compare                 & copy sign-bit to high-order bits \\
        \hline
...
\begin{tabular}{ | l | l | l | }
        ...
        \hline
        SHL      & 0x08 & Shift Left Logical \\
        \hline
        SAL      & 0x09 & Shift Left Arithmetical \\
        \hline
        SHR      & 0x0a & Shift Right Logical \\
        \hline
        SAR      & 0x0b & Shift Right Arithmetical \\
        \hline
\end{tabular}
...
\begin{tabular}{ | l | l | l | }
        ...
        \hline
        LOR      & 0x0f & Disjunction \\
        \hline
\end{tabular}

\subsection{Floating-Point Reciprocal}

        \textbf{Brief}

        Reciprocals are used for division by ``reverse multiplication.''

\begin{tabular}{ | l | l | l | }
        Mnemonic & Opcode & Brief \\
        \hline
        ARP      & 0x10   & Approximate Reciprocal
        \hline
\end{tabular}
...
\subsection{Floating-Point Multiplication}
        ...
        Multiplication; ``division'' is achieved by multiplying with a
        precalculated reciprocal approximation

        \textbf{0x02 - shift high 32-bit word into low one}

\begin{tabular}{ | l | l | l | }
        Mnemonic & Opcode & Brief \\
        \hline
        MUL      & 0x11 & Unsigned Multiplication (low 32 bits) \\
        \hline
        MUH      & 0x12 & Signed Multiplication (high 32 bits) \\
        \hline
\end{tabular}
...
\begin{tabular}{ | l | l | l | }
        ...
        Mnemonic & Opcode & Brief \\
        \hline
        BTR      & 0x14   & Bit Test and Reset \\
        \hline
        BTS      & 0x15   & Bit Test and Set \\
        \hline
        BTC      & 0x16   & Bit Test and Complement \\
        \hline
        BHW      & 0x17   & Hamming Weight / Bit Population (1-count) \\
        \hline
        BLZ      & 0x18   & Leading Zero Count \\
        \hline
        BLS      & 0x19   & Leading Sign-Bit Count \\
        \hline
        BSH      & 0x20   & 16-Bit Byte Swap \\
        \hline
        BSW      & 0x21   & 32-bit byte swap \\
        \hline
        BWL      & 0x22   & 64-bit byte swap \\
        \hline
\end{tabular}
...
\begin{tabular}{ | l | l | l | }
        Mnemonic & Opcode & Brief \\
        \hline
        LDR      & 0x20 & Load Register \\
        \hline
        STR      & 0x21 & Store Register \\
        \hline
\end{tabular}
...
\subsection{Stack}
        ...
        Mnemonic & Opcode & Brief \\
        \hline
        PSH      & 0x22 & Push Register \\
        \hline
        PSM      & 0x23 & Push Many Registers \\
        \hline
        POP      & 0x24 & Pop Register \\
        \hline
        POM      & 0x25 & Pop Many Registers \\
        \hline
\end{tabular}
...
\subsubsection{Flow Unit}
 ...
 \begin{tabular}{ | l | l | l | }
        Mnemonic & Opcode & Brief \\
        \hline
        JMP      & 0x30 & Branch Unconditionally; Absolute \\
        \hline
        JMR      & 0x31 & Branch Unconditionally; Relative \\
        \hline
        BIZ      & 0x32 & Branch If Zero \\
        \hline
        BEQ      & 0x32 & Synonymous with Branch if Zero (Equal) \\
        \hline
        BNZ      & 0x33 & Branch if Non-Zero \\
        \hline
        BNE      & 0x33 & Synonymous with Branch if Non-Zero (Not Equal) \\
        \hline
        BLT      & 0x34 & Branch if Less Than \\
        \hline
        BLE      & 0x35 & Branch if Less Than or Equal \\
        \hline
        BGT      & 0x36 & Branch if Greater Than \\
        \hline
        BGE      & 0x37 & Branch if Greater Than or Equal \\
        \hline
        BIO      & 0x38 & Branch if Overflow \\
        \hline
        BNO      & 0x39 & Branch if No Overflow \\
        \hline
        BIC      & 0x3a & Branch if Carry Set \\
        \hline
        BNC      & 0x3b & Branch if Carry Not Set \\
        \hline
        CSR      & 0x3c & Call Sub-Routine (narg) \\
        \hline
        BEG      & 0x3d & Begin Sub-Routine (nvar) \\
        \hline
        FIN      & 0x3e & Finish Sub-Routine (nvar) \\
        \hline
        RET      & 0x3f & Return From Sub-Routine (val) \\
        \hline
\end{tabular}
...
\begin{tabular}{ | l | l | l | }
        Mnemonic & Opcode & Brief \\
        \hline
        IRB      & 0x40 & Read 8-Bit Byte \\
        \hline
        IRH      & 0x41 & Read 16-Bit Half-Word \\
        \hline
        IRW      & 0x42 & Read 32-Bit Half-Word \\
        \hline
        IRL      & 0x43 & Read 64-Bit Long-Word \\
        \hline
        IWB      & 0x44 & Read 8-Bit Byte \\
        \hline
        IWH      & 0x45 & Read 16-Bit Half-Word \\
        \hline
        IWW      & 0x46 & Read 32-Bit Half-Word \\
        \hline
        IWL      & 0x47 & Read 64-Bit Long-Word \\
        \hline
        ISF      & 0x48 & Set or Clear I/O Flag \\
        \hline
        ICF      & 0x49 & Check I/O Flag \\
        \hline
\end{tabular}
...
\chapter{Instruction Set}
	...
	The V0 instruction set was designed to resemble the C language closely,
...
\begin{itemize}
	...
	\item{\textbf{m} stands for memory address operand}
\end{itemize}
	...
	This is	documented here on per-instruction basis.
        ...
        \textbf{TODO}: interrupts disabled (IDF), PF (paging enabled)

\begin{itemize}
	\item{\textbf{z} stands for zero flag (ZF)}
	\item{\textbf{c} stands for carry/borrow flag (CF)}
	\item{\textbf{o} stands for overflow flag (OF)}
	\item{\textbf{t} stands for trap flag (TF)}
	\item{\textbf{t} stands for return flag (RF)}
\end{itemize}

	\textbf{TODO}: stack/call conventions
...
\subsection{I/O Port Map}
	...
	The I/O address space maps 65,536 I/O ports to 16-bit address space.
