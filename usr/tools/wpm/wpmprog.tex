...
\title{\LARGE{Wizard Pseudo Machine} \\*
       \large{Volume One, Programmer's Guide, revision 0.0.9}}
...
\chapter{Notes}
	...
	Wizard Pseudo Machine is a software-based virtual machine. It can be
	programmed in its own instruction set / assembly dialect, using AT\&T
	syntax assembly. The incorporated assembler (\textbf{zas}) does not yet
	support	pretranslated objects, but translates assembly code into
	bytecode on-the-fly. This bytecode is then handled to the pseudo machine
	(\textbf{wpm}) for execution.

	The pseudo machine instruction set is designed to be a close match to
	basic operations of the C programming language.
