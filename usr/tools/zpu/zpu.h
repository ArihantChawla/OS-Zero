...
struct zpuop {
    ...
    unsigned src     : 5;       // 3-bit register ID + ZPU_INDIR + ZPU_INDEX
    unsigned dest    : 5;       // similar to src
    unsigned arg1    : 2;       // NONE, IMMED, ADR, REG
    unsigned arg2    : 2;       // NONE, IMMED, ADR, REG
    unsigned immed   : 9;       // small immediate constant like MSW flags
    uint32_t data[EMPTY];
} PACK();

struct zpuctx {
    int32_t regs[ZPUNREG];      // register values
