...
\title{\LARGE{ZERO VIRTUAL MACHINE} \\*
...
This text is based on an earlier document for WPM, so it's inaccurate and even
incorrect here and there. I will work on this in due time. :)

\textbf{Brief}

Zero Virtual Machine, ZVM for short, is an attempt to develop a fast virtual
machine with educational and other programming in mind. :)

I will initially make it runtime-translate and run assembly programs in AT&T
assembly with ZVM's own instruction set.

\textbf{Background and Future}

ZVM spins from a few different virtual machines I have designed and
implemented. ZVM is, at the time of writing this, a part of the OS Zero GitHub-
hosted repository at

https://github.com/vendu/OS-Zero/

Any and all feedback on Zero is welcome to be emailed to tuomoster@gmail.com
...
\chapter{Architecture}
	...
	ZVM is an architecture with 32-bit machine words in default setup,
	however, room has been left in the implementation for 64-bit suppport.

	The virtual machine supports flat 4-gigabyte address space, of which
	some is mapped for interrupt vector, interrupt handlers, thread and
	interrupt stacks, graphics, and other purposes.

	Native word size is \textbf{32-bit}. Words are machine-endian and so
	agnostic with regard to byte order.
	...
	approach, meaning there is only a single family of load-store
	instructions, (\textbf{mov}), that deals with memory-addressed operands.
...
\begin{itemize}
	\item{without virtual memory, the VM's 'physical' memory size is
	currently specified as \textbf{ZVMMEMSIZE}; this memory is mapped
	1-to-1 to virtual address space}
...
\chapter{Instruction Set}
	...
	The ZVM instruction set was designed to resemble the C language closely,
	as well as support a RISC-oriented set of typical machine operations. It
	is trivial to accelerate such operations in target assembly language.
