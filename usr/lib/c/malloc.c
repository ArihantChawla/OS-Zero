...
thrfreetls(void *arg)
{
    ...
    for (bktid = 0 ; bktid < MALLOCNBKT ; bktid++) {
        ...
        do {
            res = m_cmpsetbit((volatile long *)&bkt->ptr, MALLOC_LK_BIT_POS);
        } while (res);
        bkt->file = __FILE__;
        bkt->line = __LINE__;
        upval = (uintptr_t)arn->magbuf[bktid].ptr;
        bkt = &g_malloc.magbuf[bktid];
        upval &= ~MALLOC_LK_BIT;
        mag = (struct mag *)upval;
        if (mag) {
            mag->tab = bkt;
#if (MALLOCBUFMAG)
            n++;
            ...
            if (last) {
                ...
                magenqueue(mag, &g_malloc.magbuf[bktid]);
#elif (MALLOCTAILQ)
                magqueuemany(first, last, &g_malloc.magbuf[bktid], n);
#else
                magpushmany(first, last, &g_malloc.magbuf[bktid], n);
#endif
            } else {
#if (MALLOCLFDEQ)
                magenqueue(mag, &g_malloc.magbuf[bktid]);
        ...
        m_syncwrite(&arn->magbuf[bktid].ptr, NULL);
    }
...
#if (MALLOCATOMIC)
        mptr = &g_malloc.hdrbuf[ndx].ptr;
        do {
            res = m_cmpsetbit((volatile long *)mptr,
                              MALLOC_LK_BIT_POS);
        } while (res);
        g_malloc.hdrbuf[ndx].file = __FILE__;
        g_malloc.hdrbuf[ndx].line = __LINE__;
        mptr = &g_malloc.magbuf[ndx].ptr;
        do {
            res = m_cmpsetbit((volatile long *)mptr,
                              MALLOC_LK_BIT_POS);
        } while (res);
        g_malloc.magbuf[ndx].file = __FILE__;
        g_malloc.magbuf[ndx].line = __LINE__;
        mptr = &g_malloc.freebuf[ndx].ptr;
        do {
            res = m_cmpsetbit((volatile long *)mptr,
                              MALLOC_LK_BIT_POS);
        } while (res);
        g_malloc.freebuf[ndx].file = __FILE__;
        g_malloc.freebuf[ndx].line = __LINE__;
#else
...
#if (MALLOCATOMIC)
        mptr = &g_malloc.freebuf[ndx].ptr;
        m_cmpclrbit((volatile long *)mptr,
                    MALLOC_LK_BIT_POS);
        mptr = &g_malloc.magbuf[ndx].ptr;
        m_cmpclrbit((volatile long *)mptr,
                    MALLOC_LK_BIT_POS);
...
static struct mag *
hashfindmag(void *ptr, long act, struct mag *mag)
{
...
hashputmag(void *ptr, struct mag *mag)
{
    ...
    if (mag) {
        item = hashfindmag(ptr, MALLOCHASHPUT, mag);
    } else {
        item = hashfindmag(ptr, MALLOCHASHGET, NULL);
    }
        ...
        long zero)
{
    ...
    uint8_t       *ptr = NULL;
#if (MALLOCHDRPREFIX)
    size_t         sz = ((align <= max(MEMHDRSIZE, MALLOCALIGNMENT))
...
//    align = max(align, MALLOCALIGNMENT);
    ...
    if (mag) {
        ...
        if (lim == 1) {
            ...
            assert(ndx < mag->lim);
#endif
            adr = ((void **)mag->stk)[ndx];
        }
        if (mag->cur == lim) {
            magrmhead(bkt, mag);
        } else {
            m_cmpclrbit((volatile long *)&bkt->ptr, MALLOC_LK_BIT_POS);
        }
    } else {
        bkt = &g_malloc.magbuf[bktid];
        do {
...
#if (MALLOCDEBUG)
        assert(((uintptr_t)bkt->ptr & MALLOC_LK_BIT) == 0);
        assert(mag != NULL);
#endif
        if (mag) {
            lim = mag->lim;
...
//            _memsetbk(ptr, perturb, sz);
        ...
        ptr += max(MEMHDRSIZE, MALLOCALIGNMENT);
#endif
...
_free(void *ptr)
{
...
#endif
#elif (MALLOCARRAYHASH)
    mag = hashfindmag(ptr, MALLOCHASHGET, NULL);
#endif
#if (MALLOCDEBUG)
    assert(mag != NULL);
#endif
    if (mag) {
        ...
        bkt = mag->tab;
        do {
            res = m_cmpsetbit((volatile long *)&bkt->ptr,
                              MALLOC_LK_BIT_POS);
        } while (res);
        bkt->file = __FILE__;
        bkt->line = __LINE__;
        ...
        if (!mag->cur) {
            ...
            if ((arn) && arn->magbuf[bktid].n < magnarnbuf(bktid)) {
                ...
                magpush(mag, bkt, 1);
                       ...
                       && g_malloc.freebuf[bktid].n > magnglobbuf(bktid)) {
                /* unmap slab */
#if (MALLOCLFDEQ)
...
#elif (MALLOCARRAYHASH)
    struct mag *mag = ((ptr)
                       ? hashfindmag(ptr, MALLOCHASHFIND, NULL)
                       : NULL);
#elif (MALLOCHASH)
    struct mag *mag = ((ptr)
                       ? hashfindmag(ptr)
