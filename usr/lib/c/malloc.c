...
thrfreearn(void *arg)
{
    ...
    for (bktid = 0 ; bktid < MALLOCNBKT ; bktid++) {
        ...
        if (mag) {
            mag->adr = (void *)((uintptr_t)mag->adr | MAGGLOBAL);
            while (mag->next) {
                mag = mag->next;
                mag->adr = (void *)((uintptr_t)mag->adr | MAGGLOBAL);
                tail = mag;
...
maginitslab(struct mag *mag, long bktid)
{
    ...
    if (!mag->base) {
        ...
        if (bktid <= MALLOCSLABLOG2) {
            ...
            ptr = growheap(rounduppow2(lim * sz, PAGESIZE));
        ...
        fprintf(stderr, "allocating %ld %ld-byte blocks (%ld)\n",
                n, sz, bktid);
        mag->base = ptr;
        mag->adr = (void *)((uintptr_t)ptr | mapflg);
...
maginittab(struct mag *mag, long bktid)
{
    ...
    if (!mag->stk) {
        if (n == 1) {
            ...
            mag->ptr = &((void **)&mag->stk)[n];
#if (MALLOCFREEMAP)
            mag->freemap = &((void **)&mag->stk)[n << 1];
#endif
        } else {
            sz = magtabsz(bktid);
        ...
        long zero)
{
    ...
    uint8_t     *adr;
    uint8_t     *ptr = NULL;
    size_t       sz = 1UL << (blkbktid(size + MALLOCALIGNMENT + align));
...
//            mag->prev = NULL;
        ...
        if (!mag) {
            ...
            if (mag) {
                if (!maginit(mag, bktid)) {
                    ...
                    _assert(!mag->cur);
                    ptr = ((void **)mag->stk)[mag->cur++];
                    _assert(mag->cur == 1 && mag->cur <= mag->lim);
                    if (mag->cur < mag->lim) {
                        mag->prev = NULL;
                        mag->next = arn->magbkt[bktid].ptr;
                        if (mag->next) {
    ...
    if (ptr) {
        ...
        if (mag->lim == 1) {
            mag->freemap = ~(uintptr_t)NULL;
        } else if (bitset(mag->freemap, magptrid(mag, adr))) {
            magprint(mag);
        ...
        if (!mag->cur) {
            if (gtpow2(lim, 1)) {
                if ((mag->prev) && (mag->next)) {
                    ...
                    mag->next->prev = NULL;
                    if ((uintptr_t)mag->adr & MAGGLOBAL) {
                        g_malloc.magbkt[bktid].ptr = mag->next;
                    } else {
                        arn->magbkt[bktid].ptr = mag->next;
                    }
                } else if ((uintptr_t)mag->adr & MAGGLOBAL) {
