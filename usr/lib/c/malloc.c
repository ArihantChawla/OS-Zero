...
#if (MALLOCSPINLOCKS)
    __malloclkspin(&g_malloc.hdrbuf[bktid].lk);
#else
...
#if (MALLOCSPINLOCKS)
    __mallocunlkspin(&g_malloc.hdrbuf[bktid].lk);
#else
...
#if (MALLOCSPINLOCKS)
        __malloclkspin(&g_malloc.mlktab[ndx]);
#else
...
postfork(void)
{
    ...
    for (ndx = 0 ; ndx < MDIRNL1KEY ; ndx++) {
#if (MALLOCSPINLOCKS)
        __mallocunlkspin(&g_malloc.mlktab[ndx]);
#else
...
#if (MALLOCSPINLOCKS)
    __malloclkspin(&g_malloc.mlktab[l1]);
#else
...
#if (MALLOCSPINLOCKS)
    __mallocunlkspin(&g_malloc.mlktab[l1]);
#else
...
#else
    struct memtab  *ptab[2] = { NULL, NULL };
#endif
//    struct memtab   *ptab[3] = { NULL, NULL, NULL };
#else /* !MALLOCFREEMDIR */
    void           *ptr1;
    ...
    if (!mptr1) {
        ...
        if (mptr1 != MAP_FAILED) {
            ptab[0] = mptr1;
            g_malloc.mdir[l1].ptr = (void *)mptr1;
            mptr1->nref++;
#if defined(VALGRINDINTERNALS)
            VALGRINDALLOC(mptr1, MDIRNL2KEY * sizeof(struct memtab), 1);
#endif
#if (MALLOCSTAT)
            ntabbyte += MDIRNL2KEY * sizeof(struct memtab);
#endif
        } else {
            fail = 1;
        }
...
#if (MALLOCSTAT)
                ntabbyte += MDIRNL3KEY * sizeof(struct memtab);
#endif
#endif
                if (mptr2 != MAP_FAILED) {
                    ptab[1] = mptr2;
...
#if !defined(MDIRNL4BIT) || (!MDIRNL4BIT)
                    VALGRINDALLOC(mptr2,
                                  MDIRNL3KEY * sizeof(void *), 1);
#if (MALLOCSTAT)
                    ntabbyte += MDIRNL3KEY * sizeof(struct memtab);
#endif
#else
                    VALGRINDALLOC(mptr2,
                                  MDIRNL3KEY * sizeof(struct memtab), 1);
#endif
#endif
                } else {
                    fail = 1;
        ...
        if (!fail) {
            ...
            if (!mptr1) {
                ...
                if (mptr1 != MAP_FAILED) {
                    ...
                    mptr2->nref++;
#if defined(VALGRINDINTERNALS)
                    VALGRINDALLOC(mptr1, MDIRNL4KEY * sizeof(void *), 1);
#endif
                } else {
                    fail = 1;
                }
    ...
    if (!mag || (fail)) {
        ...
        if (mptr1) {
            if (!--mptr2->nref) {
                ...
                ntabbyte -= MDIRNL2KEY * sizeof(struct memtab);
#endif
                g_malloc.mdir[l1].ptr = NULL;
            }
        }
    }
#else /* !defined(MDIRNL4BIT) || (!MDIRNL4BIT) */
    if (!mag || (fail)) {
        mptr1 = ptab[1];
        mptr2 = ptab[0];
        if ((mptr1) && (mptr2)) {
            if (!--mptr2->nref) {
                unmapanon(mptr1, MDIRNL3KEY * sizeof(void *));
#if defined(VALGRINDINTERNALS)
                VALGRINDFREE(mptr1);
#endif
#if (MALLOCSTAT)
        ...
        if ((mptr2) && (mptr1)) {
            if (!--mptr1->nref) {
                unmapanon(mptr2, MDIRNL2KEY * sizeof(struct memtab));
#if defined(VALGRINDINTERNALS)
                VALGRINDFREE(mptr2);
#endif
#if (MALLOCSTAT)
...
mallocdiag(void)
{
    ...
    if (arn) {
        for (ndx = 0 ; ndx < MALLOCNBKT ; ndx++) {
            ...
            if (mag1) {
                if (mag1->arn != arn) {
                    fprintf(stderr, "invalid arn on arn list %ld head\n",
                            ndx);
                    magprint(mag1);

                    abort();
                } else if (mag1->prev) {
                    fprintf(stderr, "invalid prev on arn list %ld head\n",
                            ndx);
                    magprint(mag1);
                    
                    abort();
                } else if (mag1->cur >= mag1->lim) {
                    fprintf(stderr, "too high count on arn list %ld head\n");
                    magprint(mag1);
    ...
    for (ndx = 0 ; ndx < MALLOCNBKT ; ndx++) {
        ...
        if (mag1) {
            if (mag1->arn != NULL) {
                fprintf(stderr, "invalid arn on globa list %ld head\n",
                        ndx);
                magprint(mag1);
                
                abort();
            } else if (mag1->prev) {
                fprintf(stderr, "invalid prev on global list %ld head\n",
                        ndx);
                magprint(mag1);
                
                abort();
            } else if (mag1->cur >= mag1->lim) {
                fprintf(stderr, "too high count on global list %ld head\n");
                magprint(mag1);
...
#else
        __mallocinitmtx(&g_malloc.hdrbuf[ndx].lk);
#endif
        __mallocinitmtx(&g_malloc.magbkt[ndx].lk);
...
#endif /* !MALLOCNOSBRK */
#if (MALLOCMULTITAB)
    g_malloc.mlktab = mapanon(g_malloc.zerofd, MDIRNL1KEY * sizeof(MUTEX));
    if (g_malloc.mlktab == MAP_FAILED) {
        abort();
    }
    ndx = MDIRNL1KEY;
#if (MALLOCSTAT)
    ntabbyte += MDIRNL1KEY * sizeof(MUTEX);
#endif
    while (ndx--) {
#if (MALLOCSPINLOCKS)
        __mallocinitspin(&g_malloc.mlktab[ndx]);
#else
        __mallocinitmtx(&g_malloc.mlktab[ndx]);
#endif
    }
#if (MALLOCFREEMDIR)
    g_malloc.mdir = mapanon(g_malloc.zerofd,
                            MDIRNL1KEY * sizeof(struct memtab));
    if (g_malloc.mdir == MAP_FAILED) {
        abort();
    }
#if (MALLOCSTAT)
    ntabbyte += MDIRNL1KEY * sizeof(struct memtab);
#endif
#else /* !MALLOCFREEMDIR */
    g_malloc.mdir = mapanon(g_malloc.zerofd,
