...
maggethdr(long bktid)
{
    ...
    if (mag) {
        ...
        hdr = mag->next;
        if (hdr) {
            magpushmany(hdr, tail, &g_malloc.hdrbuf[bktid], 1);
        }
    }
    if (mag) {
        mag->prev = NULL;
...
thrfreetls(void *arg)
{
    ...
    for (bktid = 0 ; bktid < MALLOCNBKT ; bktid++) {
        ...
        if (mag) {
            ...
            if (tail) {
                magpushmany(first, tail, &g_malloc.magbkt[bktid], 1);
            } else {
    ...
    thrarn = NULL;
    unmapanon(thrreqstats, PAGESIZE);
    thrreqstats = NULL;
...
#if (MALLOCNBSTK)
    thrreqstats = mapanon(g_malloc.zerofd, PAGESIZE);
#endif
...
#if (MALLOCPTRNDX)
            mag->idtab = ((uint8_t *)mag->stk + n * sizeof(void *));
#else
