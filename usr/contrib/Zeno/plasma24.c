...
void drawPlasma(SDL_Surface *surface)
{
    ...
    for (y = 0; y < INTER_HEIGHT; y++) {
        ...
        for (x = 0; x < INTER_WIDTH; x++) {
            ...
            *(intermediateG + x + ypos) = colour;
            ...
            p3_sinposy += 67;
            colour = palette1[p3_palettePos - offsetTable[p3_sinposy>>7]];
    ...
    p1_xoff += 521;//1559;//1087;    // Lots of magic values. They're all prime numbers
    p1_yoff += 151;//307;     // because I figure that will make them more magical.
